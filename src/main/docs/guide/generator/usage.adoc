The source generation from JSON schema can also be used during run-time with the `io.micronaut.jsonschema.generator` package. The following code snippet shows how `SourceGenerator` object can be used to generate beans of your desired language from JSON schema.
