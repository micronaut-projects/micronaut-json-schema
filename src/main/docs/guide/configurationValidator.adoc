...
=== Custom validation rules (SoftServiceLoader)
...
Implementations are discovered using Micronaut `SoftServiceLoader` from the same classloader used to load configuration schemas.
...
To register the rule, add a service entry:
